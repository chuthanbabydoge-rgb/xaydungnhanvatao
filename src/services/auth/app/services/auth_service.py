...
class AuthService:
    ...
    async def refresh_tokens(self, refresh_token: str) -> Token:
        ...
        payload = await verify_token_async(refresh_token, token_type="refresh")
    ...
    async def logout_user(self, refresh_token: str):
        ...
        try:
            payload = await verify_token_async(refresh_token, token_type="refresh")
            # Blacklist the refresh token for 7 days (typical refresh token TTL)
            await blacklist_token(refresh_token, expires_in=604800)
        except Exception:
